...
def train():
    ...
    predictor = MovieRatingPredictor(df, 
        word2vec_params = {'vector_size': 200, 'window': 10, 'min_count': 2, 'sg': 0, 'workers': 8},                             
        model_type = 'random_forest',  # Make sure this matches the model you trained
        #model_type = 'mlp',
        n_jobs = 10
    ) 
    ...
    # Save the trained model (IMPORTANT: Do this AFTER training)
...
def train_random_forest():
    ...
    param_grid = {
        'n_estimators': [500, 1000, 2000],  # More trees (aggressive)
        'max_depth': [None, 10, 20],        # Deeper trees (careful of overfitting)
        'min_samples_split': [2, 5, 10],     # Smaller splits (aggressive)
        'min_samples_leaf': [1, 2, 4],       # Smaller leaves (aggressive)
        'max_features': ['sqrt', 'log2', None]  # Explore different feature subsets
    }

    grid_search = GridSearchCV(predictor.model, param_grid, cv=5, scoring='neg_mean_squared_error') 
...
def test():
    ...
    loaded_predictor = MovieRatingPredictor.load_model()
    print("Model loaded")
